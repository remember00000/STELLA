...
async def process_api_requests_from_file(
    ...
    api_endpoint ="completions"# api_endpoint_from_url(request_url)
    if use_ernie:
    ...
    async def call_API(
        ...
        try:
            async with aiohttp.ClientSession(trust_env=True) as session:
                # print("request_url:",request_url,"request_header:",request_header,"request_json:",self.request_json,"proxy:",proxy)
                async with session.post(
                    url=request_url, headers=request_header, json=self.request_json, proxy=proxy
                ) as response:
                    response = await response.json()
            if "error" or "error_code" in response:
                if not self.attempts_left:
                    print("error:",response)
                logging.warning(
                    f"Request {self.task_id} failed with error {response.get('error','')} {response.get('error_code', '')}"
                )
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                logging.error(f"Request  failed after all attempts. Saving errors: {self.result}")#{self.request_json}
