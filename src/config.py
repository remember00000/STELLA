...
class Config:
    ...
    position_selected: int #* positive instance position
    shuffle_neg_cands: bool  
...
def get_config(args: argparse.Namespace) -> Config:
    ...
    init_dict["data_file"] = f"topk_candidate@{args.candidate_num}_history@{args.history_num}.csv"#origin
    init_dict["data_path"] = os.path.join(C.DATA_ROOT_PATH, args.domain,  init_dict["data_file"])
    init_dict["datamaps_path"] = os.path.join(C.DATA_ROOT_PATH, args.domain, f"datamaps.json")#_cands-{args.candidate_num}

    init_dict["position_selected"]=args.position_selected
    init_dict["begin_index"] = args.begin_index
    init_dict["end_index"] = args.end_index
    init_dict["shuffle_neg_cands"]=args.shuffle_neg_cands
    init_dict["experiment_count"]=args.experiment_count
    init_dict["candidate_mode"]=args.candidate_mode
    ...
    if args.no_instruction:
        ...
        init_dict["save_path"] = os.path.join(C.RESULT_ROOT_PATH, args.domain,f"{args.experiment_count}", args.model, f"idx{args.begin_index}-{args.end_index}_posIdx@{args.position_selected}_shuffle_neg_cands@{args.shuffle_neg_cands}_example@{args.example_num}_{args.task}_candidate@{args.candidate_num}_history@{args.history_num}")
    init_dict["request_path"] = os.path.join(init_dict["save_path"], "request.jsonl")
    
    init_dict["api_url"] = C.URL_DICT[args.model]#+args.api_key
